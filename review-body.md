...
- **打开 Review**：
  ...
  - 需关注或缺件时，标题栏下列问题；点「开对话」弹出预填对话（需确认发送，不会自动执行），也可复制提示词去粘贴
  - 需关注可以点「忽略」，同一条原文之后不再列出；缺件没有忽略
- **在总览页管标签**：
  - 总览可以按标签筛选，多选为任一命中
  - 改标签写入该 skill 的 `tag.txt`，卡片和筛选立刻刷新；清空则删除该文件
- **在总览页管类型**：
  - 每个 skill 是演进型或底本型，卡片和单页名称旁显示
  - 演进型可观察使用、踩坑、实现后自发更新，必须有更新机制；每次更新告知更新标准、更新依据、更新条目
  - 底本型除信息源变更外不自发改内容，更新全部来自用户的直接命令
  - 类型写在该 skill 的 `kind.txt`（一行 `演进` 或 `底本`）；总览可按类型筛选，右键可改
- **接入 Review**：
...
3. **然后看问题**：需关注或缺件时先看标题栏；要修则点「开对话」（预填后确认发送），或点「复制」再粘贴；这条需关注可以不管则点「忽略」。
...
1. **先看芯片**：卡片和单页名称旁的演进型 / 底本型就是当前类型。
2. **再筛**：总览「类型」点演进型或底本型，只看该类；再点一次取消。
3. **然后改**：卡片右键选「标为演进型」或「标为底本型」，写入 `kind.txt` 并刷新。
